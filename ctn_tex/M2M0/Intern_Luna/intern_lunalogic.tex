...
\text{ - For fixed legs    } N\tau_iK  \\
\text{ - For floating legs } N\tau_iL_i  
\end{array}
\]
...
\text{ - For Caps    } N\tau_i(L_i-K)^+                              \\
\text{ - For Floors  } N\tau_i(K-L_i)^+
\end{array}
\]
\paragraph{Swaption}
...
\paragraph{T-forward measure} is the martingale measure $\Proba^{T}$ using zero-coupond bond $\PtT$ as numeraire. Under this probability
\[
