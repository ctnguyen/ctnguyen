...
f_{X}(x) = \frac{1}{ (2\pi)^{\frac{d}{2}} |\text{det}(\Sigma)|^{\frac{1}{2}} } \text{exp}\{ -\frac{1}{2} (x-\mu)^{t} \Sigma^{-1} (x-\mu) \}
\]
The conditional expectation of multivariate normal can be defined by partitioning $\mathbb{R}^d$
\[
X=
\left[
\begin{array}{c}
...
\]
Then the conditional expectation follow normal distribution :
\[
\mathbb{E}[X_i|X_j=x^{*}_j] \sim \mathcal{N}(\bar{\mu_{i|j}},\bar{\Sigma_{i|j}})
\text{ with }
\left\{
\begin{array}{l}
\bar{\mu_{i|j}} = \mu_i + \Sigma_{ij}\Sigma^{-1}_{jj}(x^{*}_j - \mu_j)\\ \\
\bar{\Sigma_{i|j}} = \Sigma_{jj} - \Sigma^t_{ij}\Sigma^{-1}_{ii}\Sigma_{ij}
\end{array}\right.
\]
