...
{
\pspolygon[linearc=0.2,linecolor=blue](-1.6,2.6)(3.1,2.6)(3.1,-2.1)(-1.6,-2.1)
}
\end{pspicture}
\end{center}
...
\nccurve[angleB=180]{->}{g}{MDL}\rput(0.8,0.2){\small Rebonato Approx}
\nccurve[angleA=-90]{->}{MDL}{diff}
\nccurve[angleA=90]{->}{MKT}{diff}
\nccurve[angleA=200,angleB=-90]{->}{diff}{gD}\rput(-2,-1.2){\small Minimizer}
\end{pspicture}
\end{center}
\caption{\label{fig:calibration_algorithm} General calibration algorithm : minimize  $||\text{\tiny Rebonato(gDelegate)} - \text{\tiny MKT Quotes}||^2$ }
\end{figure}

\paragraph{} From all above analysis and defined elementary tools, we are able to implement three calibration algorithm
\begin{itemize}
 \item Global
 \item Local
 \item Cascade
\end{itemize}
\paragraph{Regularized global calibration}\mbox{} \\
Regularized calibration has two penalties setting.
...
\end{pspicture}
\end{center}
\caption{\label{fig:virtual_test} Virtual Test Scheme}
\end{figure}
