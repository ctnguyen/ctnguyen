...
vv^t = \textbf{I}_d
\]
A correlated Bronian motion $\textbf{X}_t$ is similarily defined as a standard Brownia, i.e a continuous process having stationary independent increments and $\textbf{X}_t - \textbf{X}_s \sim \mathcal{N}(0,(t-s).\rho)$ where $\rho$ is the correlation matrix\footnote{Note that for being a correlation matrix, $\rho$ or $\rho^{\frac{1}{2}}$ has to satisfied somes conditions}. It is equivalent to say that $\textbf{X}_t = \rho^{\frac{1}{2}} \textbf{B}_t$ where $\rho^{\frac{1}{2}}$ is the Cholesky decomposition of $\rho$ (see~\cite{MONIQUE}1.4.3).
...
\]
\paragraph{Girsanov theorem}
\[
\text{Let }\hspace{1cm}
h\in \mathbb{L}_{\text{loc}}^2([0,T],\Omega)
\hspace{2cm}
\xi_h(t) = \text{exp}\{ \int_0^t h_s dB_s - \frac{1}{2} \int_0^t h^2_s ds \}
\]
Under the Novikov\footnote{$\mathbb{E}\text{exp}(\frac{1}{2}\int_0^T h^2_tdt) < \infty $} condition, then $\xi_h(t)$ is a $\mathbb{P}$-martingale with $\mathbb{E}[\xi_h(t)]=1$ and
\[
W_t = B_t - \int_0^t h_sdB_s \text{ is a Brownien Motion under }d\mathbb{Q} =\xi_h(T) d\mathbb{P}
\]
...
\end{itemize}
Lecturer can see detail formulations from ~\cite{ELLIOT}.ch7 , ~\cite{mathfiMarek}.ch3 or ~\cite{rouah}.  
...
Black Model used for modelling the options on future. See ~\cite{mathfiMarek} ch3.4, ~\cite{ELLIOT}.ch9
