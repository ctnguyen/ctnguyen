...
McLmm::McLmm(const Dispersion&                  dispersion,
...
Real McLmm::computeIntSto(size_t indexTime, 
						  size_t indexLibor, 
						  const std::vector<double>& G)   // nbFactor D, independent Gaussian
{
	Real varTmp = sqrt(covarianceTensor_(indexTime, indexLibor, indexLibor));  
...
void McLmm::print(const std::string& filename) const
{
	...
	//													  QLMatrix2BoostMatrix(dispersion_.get_CorrelationPtr()->get_reducedCorrelMatrixApprox())));
	//elements_print.push_back(to_print);

	//! covarianceTensor
	for(size_t indexT = 1; indexT<=horizon_; ++indexT)
	{
		PrintElement_PTR to_print      = PrintElement_PTR(new MatrixPrintElement<matrix>("covarianceTensor"+Number2String<size_t>(indexT), covarianceTensor_[indexT]));
		elements_print.push_back(to_print);
	}
